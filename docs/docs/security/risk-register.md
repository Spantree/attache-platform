...
| **Persistent autonomous access** | High | The agent has shell, file system, credential, and messaging access around the clock | Runs on a dedicated Mac mini under its own admin user. Security enforced via exec allowlists and secrets proxy, not OS-level restrictions | Switch exec to allowlist mode. Scope your 1Password vaults. Review what tools each agent actually needs. |
